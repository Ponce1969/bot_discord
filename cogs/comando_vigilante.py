...
class Vigilante(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if palabra_prohibida:
            ...
            advertencia_msg = await message.channel.send(
                f"{message.author.mention} ¡No puedes decir esa palabra!"
            )
            ...
            if message.author.id not in self.advertencias:
                self.advertencias[message.author.id] = {
                    "count": 0,
            ...
            if self.advertencias[message.author.id]["count"] >= self.max_advertencias:
                ...
                try:
                    await message.author.ban(
                        reason="Uso repetido de palabras prohibidas."
                    )
                    ban_msg = await message.channel.send(
                        f"{message.author.mention} ha sido baneado por uso repetido de palabras prohibidas."
                    )
                except discord.Forbidden:
                    ban_msg = await message.channel.send(
                        f"No tengo permisos para banear a {message.author.mention}."
                    )
                except discord.HTTPException as e:
                    ban_msg = await message.channel.send(
                        f"Ocurrió un error al intentar banear a {message.author.mention}: {e}"
                    )
