...
async def iniciar_juego(ctx):
    ...
    try:
        await esperar_mensaje(ctx, "inicio", "¡Gracias por unirte! Por favor, escribe un nombre ficticio para tu personaje:")
        nombre_ficticio = await obtener_nombre_ficticio(ctx)
        await ctx.send(f"¡Excelente, {nombre_ficticio}! Tu aventura comienza ahora. ¡Buena suerte!")
...
async def esperar_mensaje(ctx, contenido_esperado, mensaje_respuesta):
    def check(mensaje):
        return mensaje.author == ctx.author and mensaje.content.lower() == contenido_esperado
...
async def manejar_direccion(ctx, nombre_ficticio):
    ...
    None
    """
    await ctx.send(f"{nombre_ficticio}, ¡estás en el centro de la aventura! ¿Qué dirección quieres tomar? (norte, sur, este, oeste)")
    ...
    try:
        ...
        await ctx.send("¡Se acabó el tiempo para elegir una dirección! Por favor, inténtalo de nuevo.")
        await manejar_direccion(ctx, nombre_ficticio)

async def obtener_direccion(ctx):
    def check_direccion(mensaje):
        return mensaje.author == ctx.author and mensaje.content.lower() in ["norte", "sur", "este", "oeste"]
