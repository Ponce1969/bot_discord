...
def get_questions_and_answers(session: Session):
    ...
    df = pd.DataFrame(
        [(faq.question, faq.answer, faq.keyword) for faq in faqs],
...
def fuzzy_match(user_input: str, session: Session, keywords: list, min_score: int = 70):
    ...
    filtered_df = df[
        df["keyword"].apply(
            lambda x: any(
                normalize_text(keyword) in normalize_text(x) for keyword in keywords
            )
            if pd.notna(x)
            else False,
            meta=("keyword", "bool"),
    ...
    if best_match and best_match[1] >= min_score:
        answer = filtered_df[filtered_df["question"] == best_match[0]]["answer"].values[
            0
        ]
