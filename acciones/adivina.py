...
def generar_mensaje(palabra_descubierta, vidas, letras_usadas):
    return (
        f"**Palabra secreta:** {' '.join(palabra_descubierta)}\n"
...
async def adivina(ctx, bot):
    ...
    def check(m):
        return (
            m.author == ctx.author and m.channel == ctx.channel and len(m.content) == 1
        )
    ...
    while vidas > 0 and "_" in palabra_descubierta:
        ...
        if letra in palabra_secreta:
            ...
            vidas -= 1
            await ctx.send(
                f"Incorrecto. La letra '{letra}' no está en la palabra. "
                + generar_mensaje(palabra_descubierta, vidas, letras_usadas)
            )

    if "_" not in palabra_descubierta:
        await ctx.send(
            f"¡Felicidades! Has adivinado la palabra secreta: **{palabra_secreta}**"
        )
    else:
