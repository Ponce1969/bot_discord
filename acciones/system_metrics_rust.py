...
class SystemMetricsRust:
    ...
    async def get_metrics(
        self, detailed: bool = True, processes: bool = False
    ) -> dict[str, Any] | None:
        ...
        if (
            ...
            and (now - self._last_update).seconds < self.cache_duration
        ):
    ...
    async def create_advanced_embed(
        self, guild_name: str = "OrangePi 5 Plus"
    ) -> discord.Embed | None:
        ...
        **Cache:** {self.format_bytes(memory.get("cached", 0))}
        ...
        embed.add_field(
            name="💾 Almacenamiento", value=storage_info.strip(), inline=False
        )
        ...
        **Interfaces:** {len(network["interfaces"])}
        ...
        embed.add_field(name="🌐 Red", value=network_info.strip(), inline=True)
        ...
        **Arch:** {metrics["system"]["architecture"]}
        ...
        embed.add_field(name="⚙️ Sistema", value=system_info.strip(), inline=True)
        ...
        if metrics.get("processes"):
            ...
            embed.add_field(
                name="🔝 Procesos Top", value=process_info.strip(), inline=False
            )
        ...
        if metrics.get("alerts"):
            ...
            embed.add_field(name="🚨 Alertas", value=alerts_text.strip(), inline=False)
    ...
    async def create_fallback_embed(self) -> discord.Embed:
        ...
        try:
            ...
            embed.add_field(
                name="🌡️ Temperatura CPU", value=f"{cpu_temp}°C", inline=True
            )
        except Exception:
            embed.add_field(
                name="❌ Error",
...
async def create_advanced_info_embed(
    guild_name: str = "OrangePi 5 Plus",
) -> discord.Embed:
